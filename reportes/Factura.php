...
function addSociete( $nom, $adresse, $logo, $ext_logo  )
{
    ...
    $this->Image($logo, 5 , 3 , 25, 30, $ext_logo);
    $this->SetXY( $x1, $y1 );
...
function addCols($tab)
{
    ...
    // Coordenadas de la caja donde se dibujarán las columnas
    $r1 = 10;
    $r2 = $this->w - ($r1 * 2);
    $y1 = 75;
    $y2 = $this->h - 50 - $y1;

    // Dibujar un rectángulo alrededor de las columnas
    $this->SetXY($r1, $y1);
    $this->Rect($r1, $y1, $r2, $y2, "D");

    // Dibujar la línea horizontal para los encabezados
    $this->Line($r1, $y1 + 6, $r1 + $r2, $y1 + 6);

    $colX = $r1;
    $colonnes = $tab;

    // Dibujar las celdas de los encabezados
    foreach ($tab as $lib => $pos) {
        $this->SetXY($colX, $y1 + 2);
        $this->Cell($pos, 1, $lib, 0, 0, "C");
        $colX += $pos;

        // Dibujar las líneas verticales entre las columnas
...
function addLineFormat($tab)
{
    ...
    // Asignar el formato de cada columna según el array $tab
...
function lineVert($tab)
{
    ...
    // Inicializar la búsqueda del tamaño máximo de las celdas
    reset($colonnes);
    $maxSize = 0;
...
function addLine($ligne, $tab)
{
    ...
    // Dibujar cada línea de las celdas
    reset($colonnes);
    foreach ($colonnes as $lib => $pos) {
        $longCell = $pos - 2;
        $texte = $tab[$lib];
        ...
        // Establecer la posición de la celda y agregarla al PDF
        $this->SetXY($ordonnee, $ligne - 1);
        $this->MultiCell($longCell, 4, $texte, 0, $formText);

        // Actualizar el tamaño máximo de la línea
